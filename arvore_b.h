...
{ int i;
  ...
  if (p == NULL) {printf("Resultado do Teste: Arvore Vazia"); return;}
  for (i = 0; i < p->n; i++)
    TestaI(p->p[i], p->r[i].Chave, FALSE);
  TestaI(p->p[p->n], p->r[i].Chave, TRUE);
}
...
int Pesquisa_string(TipoRegistro *x, TipoApontador Ap)
{ long i = 1;
printf(("chegou\n"));
  if (Ap == NULL)
  { printf("TipoRegistro nao esta presente na arvore\n");
  ...
  printf("\n\n\n%s e %s\n\n\n", x->nome,Ap->r[i-1].nome );

  while (i < Ap->n && ( strcmp(x->nome, Ap->r[i-1].nome) )) i++; //
  if (!(strcmp(x->nome, Ap->r[i-1].nome) ))
  { *x = Ap->r[i-1];
    return Ap->r[i-1].posicao;
  }
  if (( strcmp(Ap->r[i-1].nome,x->nome ) ))  //
    Pesquisa_string(x, Ap->p[i-1]);
  else Pesquisa_string(x, Ap->p[i]);
}

void InsereNaPagina_string(TipoApontador Ap,
                    TipoRegistro Reg, TipoApontador ApDir)
{ short NaoAchouPosicao;
  int k;
  k = Ap->n;  NaoAchouPosicao = (k > 0);
  while (NaoAchouPosicao)
    { if (!(strcmp(Ap->r[k-1].nome, Reg.nome))) // strcmp(Ap->r[k-1].nome, Reg.nome)
      { NaoAchouPosicao = FALSE;
        break;
      }
      Ap->r[k] = Ap->r[k-1];
...
void Ins_string(TipoRegistro Reg, TipoApontador Ap, short *Cresceu,
         TipoRegistro *RegRetorno,  TipoApontador *ApRetorno)
{ long i = 1; long j;
  TipoApontador ApTemp;
  if (Ap == NULL)
  ...
  printf("\nPalavras %s e %s e resul comp %d"  ,Reg.nome, Ap->r[i-1].nome, strcmp(Reg.nome, Ap->r[i-1].nome) );

  while (i < Ap->n && (strcmp(Reg.nome, Ap->r[i-1].nome)))  i++; // (strcmp(Ap->r[i-1].nome, Reg.nome))
  if ((!(strcmp(Reg.nome, Ap->r[i-1].nome))))
  { printf(" Erro: Registro ja esta presente\n"); *Cresceu = FALSE;
    printf("registro:---%d---", Reg.Chave);
    system("pause");
    return;
  }
  if ((strcmp(Ap->r[i-1].nome, Reg.nome))) i--;
  Ins_string(Reg, Ap->p[i], Cresceu, RegRetorno, ApRetorno);
  if (!*Cresceu) return;
  if (Ap->n < MM)   /* Pagina tem espaco */
    { InsereNaPagina_string(Ap, *RegRetorno, *ApRetorno);
      *Cresceu = FALSE;
      return;
    }
  /* Overflow: Pagina tem que ser dividida */
  ...
  ApTemp->n = 0;  ApTemp->p[0] = NULL;
  if (i < M + 1)
  { InsereNaPagina_string(ApTemp, Ap->r[MM-1], Ap->p[MM]);
    Ap->n--;
    InsereNaPagina_string(Ap, *RegRetorno, *ApRetorno);
  }
  else InsereNaPagina_string(ApTemp, *RegRetorno, *ApRetorno);
  for (j = M + 2; j <= MM; j++)
    InsereNaPagina_string(ApTemp, Ap->r[j-1], Ap->p[j]);
  Ap->n = M;  ApTemp->p[0] = Ap->p[M+1];
  *RegRetorno = Ap->r[M];  *ApRetorno = ApTemp;
}

void Insere_string(TipoRegistro Reg, TipoApontador *Ap)
{ short Cresceu;
  TipoRegistro RegRetorno;
  TipoPagina *ApRetorno, *ApTemp;
  Ins_string(Reg, *Ap, &Cresceu, &RegRetorno, &ApRetorno);
...
void Reconstitui_string(TipoApontador ApPag, TipoApontador ApPai,
                 int PosPai, short *Diminuiu)
{ TipoPagina *Aux;  long DispAux, j;
  if (PosPai < ApPai->n)  /* Aux = TipoPagina a direita de ApPag */
  { Aux = ApPai->p[PosPai+1];  DispAux = (Aux->n - M + 1) / 2;
    ...
    if (DispAux > 0)  /* Existe folga: transfere de Aux para ApPag */
    { for (j = 1; j < DispAux; j++)
        InsereNaPagina_string(ApPag, Aux->r[j-1], Aux->p[j]);
      ApPai->r[PosPai] = Aux->r[DispAux-1];  Aux->n -= DispAux;
      for (j = 0; j < Aux->n; j++) Aux->r[j] = Aux->r[j + DispAux];
      for (j = 0; j <= Aux->n; j++) Aux->p[j] = Aux->p[j + DispAux];
      *Diminuiu = FALSE;
    ...
    { Aux = ApPai->p[PosPai-1]; DispAux = (Aux->n - M + 1) / 2;
      ...
      if (DispAux > 0) /* Existe folga: transf. de Aux para ApPag */
      { for (j = 1; j < DispAux; j++)
          InsereNaPagina_string(ApPag, Aux->r[Aux->n - j],
	                 Aux->p[Aux->n - j + 1]);
        ApPag->p[0] = Aux->p[Aux->n - DispAux + 1];
        ApPai->r[PosPai-1] = Aux->r[Aux->n - DispAux];
        Aux->n -= DispAux;  *Diminuiu = FALSE;
        ...
        { for (j = 1; j <= M; j++)
            InsereNaPagina_string(Aux, ApPag->r[j-1], ApPag->p[j]);
          free(ApPag);  ApPai->n--;
          if (ApPai->n >= M)  *Diminuiu = FALSE;
        }
    }
...
void Antecessor_string(TipoApontador Ap, int Ind,
                TipoApontador ApPai, short *Diminuiu)
{ if (ApPai->p[ApPai->n] != NULL)
  { Antecessor_string(Ap, Ind, ApPai->p[ApPai->n], Diminuiu);
    if (*Diminuiu)
    Reconstitui_string(ApPai->p[ApPai->n], ApPai, (long)ApPai->n, Diminuiu);
    return;
  }
  Ap->r[Ind-1] = ApPai->r[ApPai->n - 1];
  ApPai->n--;  *Diminuiu = (ApPai->n < M);
...
void Ret_string(char *Ch, TipoApontador *Ap, short *Diminuiu)
{ long j, Ind = 1;
  TipoApontador Pag;
  if (*Ap == NULL)
  { printf("Erro: registro nao esta na arvore\n"); *Diminuiu = FALSE;
  ...
  Pag = *Ap;
  while (Ind < Pag->n && (strcmp(Ch, Pag->r[Ind-1].nome))) Ind++;  // (strcmp(Ch, Pag->r[Ind-1].nome))
  if (!(strcmp(Ch, Pag->r[Ind-1].nome)))
  { if (Pag->p[Ind-1] == NULL)   /* TipoPagina folha */
    { Pag->n--;
      *Diminuiu = (Pag->n < M);
      for (j = Ind; j <= Pag->n; j++)
    ...
    /* TipoPagina nao e folha: trocar com antecessor */
    Antecessor_string(*Ap, Ind, Pag->p[Ind-1], Diminuiu);
    if (*Diminuiu)
    Reconstitui_string(Pag->p[Ind-1], *Ap, Ind - 1, Diminuiu);
    return;
  }
  if (strcmp(Ch, Pag->r[Ind-1].nome)) Ind++; //strcmp(Ch, Pag->r[Ind-1].nome)
  Ret_string(Ch, &Pag->p[Ind-1], Diminuiu);
  if (*Diminuiu) Reconstitui_string(Pag->p[Ind-1], *Ap, Ind - 1, Diminuiu);
}

void Retira_string(char *Ch, TipoApontador *Ap)
{ short Diminuiu;
  TipoApontador Aux;
  Ret_string(Ch, Ap, &Diminuiu);
  if (Diminuiu && (*Ap)->n == 0)  /* Arvore diminui na altura */
  { Aux = *Ap;   *Ap = Aux->p[0];
    free(Aux);
  }
...
void ImprimeI_string(TipoApontador p, int nivel)
{ long i;
  if (p == NULL) {return;}
  printf("Nivel %d : ", nivel);
  for (i = 0; i < p->n; i++)
  ...
  nivel++;
  for (i = 0; i <= p->n; i++)
    ImprimeI_string(p->p[i], nivel);
}

void Imprime_string(TipoApontador p)
{ int  n = 0; ImprimeI_string(p, n); }

void TestaI_string(TipoApontador p, char *pai, short direita)
{ int i,j;
  char antecessor[250];
  for (j=0; j<=250; j++)
    antecessor[j] = 0;
  ...
  if (p == NULL) return;
  if ((strcmp(p->r[0].nome, pai)) && direita == FALSE)// (strcmp(p->r[i].Chave,antecessor))
  { printf("Erro: filho %s maior que pai %s\n", p->r[0].nome, pai);
    return;
  }
  for (i = 0; i < p->n; i++)
    { if (!(strcmp(p->r[i].nome,antecessor)))
      { printf("Erro: irmao %s maior que irmao a esquerda %s\n",
        p->r[i].nome, antecessor);
        return;
      }
      strcpy(antecessor,p->r[i].nome );

    }
  for (i = 0; i < p->n; i++)
    TestaI_string(p->p[i], p->r[i].nome, FALSE);
  TestaI_string(p->p[p->n], p->r[i].nome, TRUE);
}

void Testa_string(TipoApontador p)
{ int i;
